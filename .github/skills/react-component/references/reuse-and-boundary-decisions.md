...
| --- | --- |
| Pure JSX from props, server translations/data, no hooks/browser/event handler | Keep server-compatible |
...
| Existing cross-route cache/preferences | Existing Zustand selector | `sites/arolariu.ro/src/stores/`; load the `zustand-store` skill |
...
| Fetch because a client-only changing identifier owns stale-while-revalidate | Existing client hook pattern with cancellation/stale-result protection as needed |
