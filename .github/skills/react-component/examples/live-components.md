...
It derives filtered/paginated output with memoization and event-owned setters
without an effect that mirrors props into state.

### Inspect

Separate intent state from derivable output and verify edge cases. Do not copy
its generic JSON search when the feature has a domain-specific searchable
contract.
