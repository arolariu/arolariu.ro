---
name: refactor
description: Improve structure without changing observable behavior. Use for explicit requests to extract, split, move, consolidate, rename, or reduce coupling after behavior is characterized.
---
...
- A defect must first be reproduced and fixed; use the `fix-bug` skill.
- A package or framework must change; use the `dependency-migration` skill.
...
3. Select a characterization strategy and establish a passing baseline before
   ...
   the behavior.
4. Select one catalogued transformation and inspect a current sibling with the
