# Dependency Update Rollback Checklist
...
- [ ] Restore the exact prior `uses:` ref in every approved workflow or
      composite-action caller; do not normalize unrelated action-version drift.
- [ ] Restore any action-owned input/output, permissions, cache, artifact, or
      runner/runtime adaptations in reverse order.
- [ ] Re-run the same workflow validation used for the approved update and
      confirm local action references and producer/consumer pairs still align.
