...
- Use `fix-bug` when production behavior is defective and must change.
- Use `fix-bug` when fail-without/pass-with proof or any production correction
  is required.
...
- Root and local testing guidance plus the matching TypeScript, React,
  frontend, C#, or backend instructions.
...
3. Which frontend or backend behavior categories are relevant; do not turn a
   matrix into indiscriminate case generation.
...
7. If the new test instead exposes a production defect, stop and route the
   evidence to `fix-bug`. Never mutate correct production code or create an
...
| Before choosing Vitest or Testing Library behavior cases | [Frontend test matrix](references/frontend-test-matrix.md) |
| Before choosing MSTest behavior cases | [Backend test matrix](references/backend-test-matrix.md) |
...
- Frontend tests are colocated `*.test.ts`/`*.test.tsx`, use the configured
  environment, and assert user-visible or public outcomes.
- Backend tests use the current MSTest class/method shape and exact-type
  exception assertions when exception classification is the contract.
