# Server Component Troubleshooting
...
| --- | --- | --- |
| Typed selector rejects a new key or generated message declarations are stale | The `react-internationalization` skill and affected locale subtree | Restore source dictionary/ICU parity through the i18n owner, then regenerate; never edit the declaration by hand. |
...
| A Server Action succeeds but the page receives malformed values | `react-server-action`, its `response.json()` boundary, and domain transport parser | Keep payload `unknown`, parse it, and map parser failure to the typed result before rendering. |
