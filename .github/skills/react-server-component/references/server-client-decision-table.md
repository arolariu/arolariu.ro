...
| Mutation or authenticated API call initiated by the client | Existing/new `"use server"` action through `react-server-action` | `sites/arolariu.ro/src/app/domains/invoices/_actions/invoices/fetchInvoice.ts`; `sites/arolariu.ro/src/lib/utils.server.ts` |
| Browser-only stream, worker, clipboard, observer, or device API | `react-client-component` for markup; `react-client-hook` for reusable lifecycle logic | `sites/arolariu.ro/src/app/playground/workers/island.tsx`; `sites/arolariu.ro/src/hooks/useScrollToTop.tsx` |
...
| Existing cross-route client cache/preferences | Existing Zustand store, after inspecting its hydration contract | `sites/arolariu.ro/src/stores/`; use the `react-client-store` skill |
...
- Server Action failures remain typed results owned by
  `react-server-action`. Map them to not-found, forbidden, retryable error, or
  inline state at the consuming route boundary.
...
- invoke `react-auth` and stop for approval before changing the matcher, access
  rules, redirect behavior, or data visibility.
