...
async function getFileSizesFromGit(params, branchName, targetFolders) {
    ...
    for (const folder of targetFolders) {
        try {
            ...
            const { stdout, stderr, exitCode } = await exec.getExecOutput(`git ls-tree -r -l ${branchName} -- ${folderPath}`, [], { ignoreReturnCode: true, silent: true });
            if (exitCode !== 0) {
                ...
                if (stderr && stderr.includes("fatal: Not a valid object name")) {
                    core.warning(`Branch '${branchName}' or path '${folderPath}' might not exist or was not fetched correctly for bundle size check.`);
                }
                continue;
            }
...
async function getJestResultsSection(core) {
    ...
    try {
        ...
        const jestSummary = JSON.parse(jestSummaryRaw);
        const { numTotalTestSuites, numPassedTestSuites, numFailedTestSuites, numPendingTestSuites, } = jestSummary;
        const { numTotalTests, numPassedTests, numFailedTests, numPendingTests } = jestSummary;
        const startTime = new Date(jestSummary.startTime);
        section += `**Summary:**\n`;
        section += `*   **Test Suites:** ${numPassedTestSuites} passed, ${numFailedTestSuites} failed, ${numPendingTestSuites ?? 0} pending, ${numTotalTestSuites} total\n`;
        section += `*   **Tests:**       ${numPassedTests} passed, ${numFailedTests} failed, ${numPendingTests ?? 0} pending, ${numTotalTests} total\n`;
        section += `*   **Start Time:**  ${startTime.toLocaleString()}\n`;
        section += `*   **Overall:**     ${jestSummary.success ? "✅ Passed" : "❌ Failed"}\n\n`;
        if (numFailedTests > 0 && jestSummary.testResults) {
            ...
            for (const testResult of jestSummary.testResults) {
                if (testResult.status === "failed" && testResult.assertionResults) {
                    const suitePath = testResult.name.replace(process.env.GITHUB_WORKSPACE ?? "", "");
        ...
        const err = error;
        core.warning(`Could not read or parse Jest summary from ${jestSummaryPath}: ${err.message}`);
        ...
        for (const folder of targetFolders) {
            ...
            for (const path of relevantFilePaths) {
                ...
                if (mainSize !== undefined)
                    folderMainTotalSize += mainSize;
                if (previewSize !== undefined)
                    folderPreviewTotalSize += previewSize;
                let status = "";
                let diff = 0;
                let changed = false;
            ...
            if (numFilesChanged > 0 || folderDiff !== 0) {
                ...
                const diffDisplay = folderDiff === 0
                    ? "---"
                    : `${diffSign}${(0, pretty_bytes_1.default)(Math.abs(folderDiff))}`;
                let folderStatusText = "Modified";
                if (numFilesChanged === 0 && folderDiff !== 0)
                    folderStatusText = "Size Changed";
                else if (folderPreviewTotalSize === 0 &&
                    folderMainTotalSize > 0 &&
                    numFilesChanged === relevantFilePaths.size)
                    folderStatusText = "Removed";
                else if (folderMainTotalSize === 0 &&
                ...
                section += `<details>\n`;
                section += `<summary><strong>\`${folder}\`</strong> - Total Diff: ${diffDisplay} (Preview: ${(0, pretty_bytes_1.default)(folderPreviewTotalSize)} vs Main: ${(0, pretty_bytes_1.default)(folderMainTotalSize)}) - ${numFilesChanged} file(s) changed (${folderStatusText})</summary>\n\n`;
                if (numFilesChanged > 0) {
                    ...
                    for (const item of filesInFolder) {
                        ...
                        const itemDiffDisplay = item.diff === 0
                            ? "---"
                ...
                section += `<details>\n`;
                section += `<summary><strong>\`${folder}\`</strong> - No changes (Preview: ${(0, pretty_bytes_1.default)(folderPreviewTotalSize)}, Main: ${(0, pretty_bytes_1.default)(folderMainTotalSize)})</summary>\n`;
...
exports.default = async (params) => {
    ...
    commentBody += await getBranchCommitComparisonSection(params, currentCommitSha, shortCurrentCommitSha);
