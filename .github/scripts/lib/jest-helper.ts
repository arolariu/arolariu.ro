...
async function getCoverageFromJson(
  ...
  try {
    ...
    const filePaths = Object.keys(coverageSummaryData).filter(
      (k) => k !== "total" && coverageSummaryData[k]
    );
    filePaths.sort(); // Sort for consistent output

    for (const filePath of filePaths) {
      table += formatCoverageRow(
        filePath,
        coverageSummaryData[filePath],
        false
      );
    }
    ...
    core.warning(
      `Could not read or parse Jest coverage summary from ${coverageJsonPath}: ${err.message}`
    );
...
export default async function getJestResultsSection(
  ...
  const coverageTable = await getCoverageFromJson(
