...
import {createCommentBuilder, createGitHubHelper, env, git, MarkdownUtils} from "../helpers/index.ts";
 ...
 */
const BUNDLE_DIRECTORIES = ["sites/arolariu.ro/.next/static", "sites/arolariu.ro/dist", "packages/components/dist"];
...
function buildReportComment(report: BundleReport, baseRef: string, headRef: string): string {
  ...
  if (report.files.length > 10) {
    ...
    builder.addCollapsible(`All Files (${report.files.length} total)`, allFilesList.join("\n"), true);
  }
...
export default async function generateBundleSizeReport(): Promise<void> {
  try {
    ...
    const headRef = env.get("HEAD_REF", "HEAD") ?? "HEAD";
