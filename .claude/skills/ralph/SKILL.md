...
- --parallel: concurrent insight workers (max 5) + cross-connect validation
- --batch [id]: process only tasks from specific batch
- --type [type]: process only tasks at a specific phase (extract, create, connect, refresh, validate, enrich)
...
|-------|---------------|---------|
| extract | /distill | distill insights from source material |
| create | (inline insight creation) | Write the insight file |
| enrich | /enrich | Add content to existing insight |
| connect | /connect | Find connections, update domain maps |
| refresh | /refresh | Update older {DOMAIN:note_plural} with new connections |
| validate | /validate | Description quality + schema + health checks |
...
```yaml
phase_order:
  insight: [create, connect, refresh, validate]
  enrichment: [enrich, connect, refresh, validate]
...
Phase distribution:
  Claims:       {create: N, connect: N, refresh: N, validate: N}
  Enrichments:  {enrich: N, connect: N, refresh: N, validate: N}
...
Run /distill --handoff on the source file referenced in the task file.
After extraction: create per-insight task files, update the queue with new entries
(1 entry per insight with current_phase/completed_phases), output RALPH HANDOFF.
...
You are processing task {ID} from the work queue.
Phase: create | Target insight: {TARGET}

Create a insight for this insight in insights/[insight as sentence].md
Follow insight design patterns:
...
- Footer: Source (wiki link), Relevant Insights (with context), Topics
...
The task file specifies which existing insight to enrich and what to add.
ONE PHASE ONLY. Do NOT run connect.
```

For **connect** phase:

**Build sibling list:** Query the queue for other insights in the same batch where `completed_phases` includes "create" (insight already exists). Format as wiki links.
...
- [[{SIBLING_TARGET}]]
{end for, or "None yet" if this is the first insight}

Run /connect --handoff on: {TARGET}
Use dual discovery: domain map exploration AND semantic search.
Add inline links where genuine connections exist — including sibling insights listed above.
Update relevant domain map with this insight.
ONE PHASE ONLY. Do NOT run refresh.
```
...
Run /refresh --handoff for: {TARGET}
This is the BACKWARD pass. Find OLDER {DOMAIN:note_plural} AND sibling insights
that should reference this insight but don't.
Add inline links FROM older {DOMAIN:note_plural} TO this insight.
ONE PHASE ONLY. Do NOT run validate.
```
...
You are processing task {ID} from the work queue.
Phase: validate | Target: {TARGET}

Run /validate --handoff on: {TARGET}
...
Final phase for this insight. ONE PHASE ONLY.
```
...
4. **Capture learnings:** If Learnings section has non-NONE entries, insight them for the final report
...
**If the last phase** (validate) — mark task done:
- Set `status: done`
- Set `completed` to current UTC timestamp
- Set `current_phase` to null
- Append the completed phase to `completed_phases`

**For extract tasks ONLY:** Re-read the queue after marking done. The distill skill writes new task entries (1 entry per insight/enrichment with `current_phase`/`completed_phases`) to the queue during execution. The lead must pick these up for subsequent iterations.
...
After advancing a task to "done" (Step 4e), check if ALL tasks in that batch now have `status: "done"`. If yes and the batch has 2 or more completed insights:

1. **Collect all insight paths** from completed batch tasks. For each insight task with `status: "done"`, read the task file's `## Create` section to find the created insight path.
...
Insights created in this batch:
{list of ALL insight titles + paths from completed batch tasks}

Validate sibling connections exist between batch insights. Add any that were missed
because sibling insights did not exist yet when the earlier insight's connect ran.
...
**Skip if:** batch has only 1 insight (no siblings) or tasks from the batch are still pending.
...
**Incompatible flags:** `--parallel` cannot be combined with `--type`. Parallel mode processes insights end-to-end (all phases). If `--type` is also set, report an error:
```
ERROR: --parallel and --type are incompatible. Parallel processes full insight pipelines, not individual phases.
Use serial mode for per-phase filtering: /ralph N --type connect
```

### Parallel Architecture

**Two-phase design:** Workers receive sibling insight info upfront so they can link proactively. Phase B validates and catches any gaps.

```
Ralph Lead (you) — orchestration only
|
+-- PHASE A: PARALLEL CLAIM PROCESSING (concurrent)
|   +-- worker-001: all 4 phases for insight 001 (with sibling awareness)
|   +-- worker-002: all 4 phases for insight 002 (with sibling awareness)
|   +-- worker-003: all 4 phases for insight 003 (with sibling awareness)
...
**Why two phases?** Workers have sibling awareness (insight titles in spawn prompt) and link proactively during connect/refresh. But timing means some sibling insights may not exist yet during a worker's connect phase. Phase B runs a single cross-connect pass after all insights exist.

### 6a. Identify Parallelizable Claims

From the filtered queue, find pending insights. A insight is parallelizable when its `status == "pending"`. Cap at 5 concurrent workers (or N, whichever is smaller).

Report:
```
=== Parallel Mode ===
Parallelizable insights: {count}
Max concurrent workers: {min(count, N, 5)}
```

### 6b. Spawn Claim Workers

For each parallelizable insight (up to N requested, max 5 concurrent):

Build the worker prompt with sibling awareness:

```
You are a insight worker processing insight "{TARGET}" from batch "{BATCH}".
...
{for each other insight in the batch:}
- "{SIBLING_TARGET}" (task file: ops/queue/{SIBLING_FILE})
{end for}

During REFLECT and REWEAVE, check if your insight genuinely connects to any sibling.
If a sibling insight exists in insights/, link to it inline where the
...
insight title, path, and insight ID. The lead needs this for cross-connect.
```
...
Task(
  ...
  description = "insight: {short target}" (5 words max)
)
```
...
1. **Parse completion message** — extract the created insight title and path (needed for Phase B)
2. **Log any learnings** from the worker's report
3. **Check for issues** — failures, skipped phases, resource conflicts

**Collect all created insights** — maintain a list of `{note_title, note_path}` from worker completion messages. You need this for the cross-connect validation phase.
...
**Skip if only 1 insight was processed** (no siblings to cross-connect).
...
Insights created in this batch:
{list of ALL newly created insight titles with paths from Phase A}

Validate sibling connections exist between these insights. Add any connections that
workers missed because sibling insights did not exist yet when a worker's connect ran.
Check backward link gaps. Output RALPH HANDOFF block when done.",
  description = "cross-connect: batch {BATCH}"
)
```
...
Queue state:
  Pending: {count}
  Done: {count}
  Phase distribution: {create: N, connect: N, refresh: N, validate: N}
...
For extract tasks: if zero insights extracted, log as an observation. Do NOT retry automatically.

### Gate 4: Task File Updated
After each phase, the task file's corresponding section (Create, Connect, Refresh, Validate) should be filled. If empty after subagent completes, log warning.
...
- Include sibling insight titles in connect and refresh prompts
...
- Validate subagent count equals task count in final report
