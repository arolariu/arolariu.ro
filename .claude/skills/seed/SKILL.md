...
- **Size**: line count (affects chunking decisions in /distill)
...
```
mcp__qmd__search query="insights from {source filename}" limit=5
```

Or via keyword search in the insights/ directory:
```bash
grep -rl "{key terms from source title}" insights/ 2>/dev/null | head -5
```
...
**Why move immediately:** All references (task files, {DOMAIN:note_plural}' Source footers) use the final archived path from the start. No path updates needed later. If it is in inbox, it is unclaimed. Claimed sources live in archive.

## Step 5: Determine Claim Numbering

Find the highest existing insight number across the queue and archive to ensure globally unique insight IDs.

```bash
# Check queue for highest insight number in file references
QUEUE_MAX=$(grep -oE '[0-9]{3}\.md' ops/queue*.yaml ops/queue/*.yaml 2>/dev/null | \
  grep -oE '[0-9]{3}' | sort -n | tail -1)
QUEUE_MAX=${QUEUE_MAX:-0}

# Check archive for highest insight number
...
# Next insight starts after the highest
NEXT_CLAIM_START=$((QUEUE_MAX > ARCHIVE_MAX ? QUEUE_MAX + 1 : ARCHIVE_MAX + 1))
```

Claim numbers are globally unique and never reused across batches. This ensures every insight file name (`{source}-{NNN}.md`) is unique vault-wide.
...
next_insight_start: {NEXT_CLAIM_START}
---
...
## Acceptance Criteria
- distill insights, implementation ideas, tensions, and testable hypotheses
- Duplicate check against insights/ during extraction
...
Next steps:
  /ralph 1 --batch {SOURCE_BASENAME}     (distill insights)
...
- Handles insight numbering automatically (globally unique)
...
**Large source (2500+ lines):** Insight in output: "Large source ({N} lines) -- /distill will chunk automatically."
...
- Reuse insight numbers from previous batches (globally unique is required)
...
- Compute next_insight_start from both queue AND archive (not just one)
