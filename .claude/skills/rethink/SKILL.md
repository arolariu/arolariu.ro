...
3. **`ops/methodology/`** — existing methodology insights (read all to understand current system self-knowledge)
...
Every rule in the context file, every workflow in a skill, every assumption baked into the architecture was a hypothesis at some point. Hypotheses need testing against reality. Observation insights in `ops/observations/` capture friction from actual use. Tension insights in `ops/tensions/` capture unresolved conflicts. Rethink first triages these individually (some become insights, some become methodology updates, some get archived), then compares remaining evidence against what the system assumes and proposes changes when patterns emerge.
...
```bash
# Get all methodology insights with their metadata
...
Read all methodology insights fully. Extract:
- Each insight's category, created date, updated date, status
- The behavioral assertions each insight makes (the "What to Do" sections)
...
```bash
# Compare config.yaml modification time vs newest methodology insight
...
For each active feature in config.yaml (features with `enabled: true` or features present in the active configuration), check whether a corresponding methodology insight exists. Features without methodology coverage represent gaps — the system does things it cannot explain to itself.

Check these feature areas:
- Processing pipeline (is there a methodology insight about processing behavior?)
- Maintenance conditions (methodology insights about when maintenance triggers?)
- Session rhythm (methodology insights about session workflow?)
- Domain-specific behaviors (methodology insights about domain vocabulary and patterns?)

**Type 3: Assertion Mismatch**

For each methodology insight that makes a behavioral assertion ("What to Do" section), check:
...
- Are there other methodology insights that contradict this one?

Report: which assertions align, which contradict, which have no corresponding system element.

### 0d. Create Drift Observations

For each drift finding, create an observation insight in `ops/observations/`:
...
related_insights: ["[[methodology insight]]", "[[config element]]"]
---
# [drift finding as prose sentence]

**Drift type:** staleness | coverage-gap | assertion-mismatch
**Methodology insight:** [[affected insight]]
...
Resolution: update methodology insight | update system config | flag for human review
```
...
Drift Check:
  ...
  Coverage gaps: [N features without methodology insights]
...
Read each pending item fully. These are small atomic insights — load all of them. Understanding the full content is required for accurate triage. If zero pending items, report clean state and exit early.

Also read `ops/methodology/` to understand existing methodology insights — this prevents creating duplicates and informs whether new observations should extend existing methodology rather than create new insights.
...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent insight | General principle across sessions. Would work as a insight insight. Crystallized insight, not operational guidance. | Create insight in insights/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
**For PROMOTE items:**
1. Create insight with prose-as-title in insights/
2. Follow standard insight schema: YAML frontmatter (description, type, created), body developing the insight, Topics footer linking to relevant domain map(s)
3. The observation content becomes the seed for the insight body — but develop it fully, do not just copy the observation
4. Update the observation: set `status: promoted`, add `promoted_to: [[insight title]]`
...
**Update domain maps:** After triage execution, update `ops/observations.md` and `ops/tensions.md` to connect status changes. Move entries between Pending/Promoted/Archived/Resolved/Dissolved sections as appropriate.

---

## Phase 2: Methodology Folder Updates

For items triaged as METHODOLOGY, create or update insights in `ops/methodology/`.

### Creating New Methodology Insights
...
### Extending Existing Methodology Insights

If a methodology insight with similar content already exists:
1. Do NOT create a duplicate
2. Instead, add the new evidence to the existing insight
...
Before creating a new methodology insight:
1. Read all files in `ops/methodology/` (these are small)
2. Check if any existing insight covers the same behavioral area
3. If overlap > 80%, extend rather than duplicate

### Update Methodology domain map

After creating or updating methodology insights, update `ops/methodology.md`:
- Add new insights to the appropriate category section
- Update context phrases for modified insights
...
4. **Methodology insights** — patterns in `ops/methodology/` by category
...
| Methodology convergence | Multiple /remember captures in ops/methodology/ pointing at the same behavioral pattern | Methodology insight needs elevation to context file | A methodology learning has been validated enough to become a system-level rule |
...
2. **Group by referenced domain maps or system areas:** Extract wiki links and file references from observation bodies. 3+ observations referencing the same area = recurring theme.
...
5. **Compare methodology insights against context file:** If `ops/methodology/` has 3+ insights in the same category that are not reflected in the context file, the methodology has converged enough for elevation.
...
|-------------------|----------------------|
| 2 observations, same area | Methodology insight update |
...
### Promoted Insights Need Connections

If any observations were promoted to insights:

```
  [count] insights were promoted from observations.
  Run /connect on promoted insights to find connections.
  Promoted: [list of insight titles]
```

### Pipeline Queue Integration

If promoted items should enter the processing pipeline (queue-based systems):
- Add each promoted insight to the queue with `current_phase: "connect"` (the insight already exists, so skip create)
...
2. Recommend creating them: "The operational learning loop requires `ops/observations/` and `ops/tensions/`. Create these directories and their domain map files to begin capturing system friction."
...
Run triage normally but insight that pattern detection requires more data:
```
  Insight: [N] items is below the threshold for reliable pattern detection.
...
- Update domain maps after triage changes status of observations/tensions
