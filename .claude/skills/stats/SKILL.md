...
1. **`ops/derivation-manifest.md`** — vocabulary mapping
   ...
   - Use `vocabulary.insights_collection` for semantic search collection name
...
```bash
# Orphan count (insights with zero incoming links)
ORPHAN_COUNT=0
...
# Schema compliance (% of insights with required fields: description, topics)
...
if [[ "$TOTAL_FILES" -gt 0 ]]; then
  # Insights with BOTH required fields
...
# Processed ratio (insights vs inbox)
...
```bash
# This week's growth (insights with created: date within last 7 days)
...
# This week's connections (approximate — count links in recently created insights)
...
--=={ stats }==--
  ...
  ===============
  insights:  [NOTE_COUNT]
  Connections:               [LINK_COUNT] (avg [AVG_LINKS] per insight)
  domain maps:   [domain map_COUNT] (covering [COVERAGE]% of insights)
  Topics:                    [TOPIC_COUNT]
  ...
  Growth
  ======
  This week:    +[THIS_WEEK_NOTES] insights, +[THIS_WEEK_LINKS] connections
...
| Condition | Insight |
|-----------|------|
| ORPHAN_COUNT > 0 | "[N] orphan insights — run `/graph health` for details" |
| DANGLING_COUNT > 0 | "[N] dangling links — run `/graph health` to identify broken links" |
| COMPLIANCE < 90 | "Schema compliance below 90% — some insights missing required fields" |
...
| PROCESSED_PCT < 50 | "More content in inbox than in insights/ — consider processing backlog" |
| THIS_WEEK_NOTES == 0 | "No new insights this week" |

Only show interpretation insights when conditions are notable. A healthy vault gets just the stats, no warnings.
...
- **[NOTE_COUNT]** insights with **[LINK_COUNT]** connections (avg [AVG_LINKS] per insight)
- **[domain map_COUNT]** domain maps covering [COVERAGE]% of insights
...
Use universal vocabulary (insights, domain maps, etc.). All metrics work identically.

### Very Large Vault (500+ insights)

The orphan and domain map coverage checks may be slow for large vaults. If insights/ has >200 files:
1. Run orphan detection with a simpler heuristic (check only for presence in any domain map, not full backlink scan)
2. Insight: "Metrics approximate for large vault. Run /graph health for precise analysis."
