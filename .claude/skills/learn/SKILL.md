...
ops/derivation-manifest.md  — domain vocabulary (inbox folder, distill skill name)
```
...
- Distill skill name: `/distill` (could be `/surface`, `/break-down`, etc.)
- Domain name and hub domain map name
...
Format for downstream distill extraction — findings as clear propositions, not raw dumps:
...
should be a clear proposition the distill phase can extract as an atomic insight.]
...
Research complete
  ...
  Next: /[distill-skill-name] [inbox-folder]/[filename]
```

Append based on mode:
- **manual:** (nothing extra)
- **suggested:** `Ready for processing when you are.`
- **automatic:** Replace "Next" line with `Queued for /[distill-skill-name] -- processing will begin automatically.`
...
| Phase | Skill | Purpose |
|-------|-------|---------|
| distill insights | /[distill-name] | Mine research for atomic propositions |
| Find connections | /[connect-name] | Link new insights to existing graph |
| Update old insights | /[refresh-name] | Backward pass on touched insights |
| Quality check | /[validate-name] | Description quality, schema, links |
