---
name: pipeline
description: End-to-end source processing -- seed, distill, process all insights through connect/refresh/validate, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Phase 1: /seed — create extract task, move source to archive
    |
    v
Phase 2: /distill (via /ralph) — distill insights from source
    |
    v
Phase 3: /ralph (all insights) — create -> connect -> refresh -> validate
    |
    v
...
- **next_insight_start**: the insight numbering start
...
## Phase 2: Extract (Distill)

Process the extract task via /ralph. This spawns a subagent that runs /distill, extracting insights from the source and creating task entries in the queue.
...
After completion, read the queue to count extracted insights and enrichments:

Check how many pending tasks exist for this batch. The distill phase creates 1 queue entry per insight and 1 per enrichment.
...
**If zero insights extracted:** Report the issue. For TFT sources, zero extraction is a bug — the source almost certainly contains extractable content. Ask the user whether to retry with different scope or skip.
...
This processes every insight through: create -> connect -> refresh -> validate. And every enrichment through: enrich -> connect -> refresh -> validate.
...
```
$ Processing insight 1/{total}: {title}
  $ create... done
  $ connect... done (3 connections found)
  $ refresh... done (2 {DOMAIN:note_plural} updated)
  $ validate... done (PASS)
```

**For large batches (20+ insights):** /ralph handles context isolation automatically via subagents. The pipeline does NOT need to chunk — /ralph processes N tasks sequentially with fresh context per phase.

---

## Phase 4: Validate Completion

After /ralph finishes, validate all tasks for this batch are done.
...
Processing:
  ...
  Connections added: {C}
  domain maps updated: {T}
  Older {DOMAIN:note_plural} updated via refresh: {R}

Quality:
  All validate checks: {PASS/FAIL count}
...
- Processed all insights through 4-phase pipeline
- Archived batch to {archive_path}

Files Modified:
- insights/ ({N} new {DOMAIN:note_plural})
...
**Extract failure:** If /distill extracts zero insights, report and stop. Do not proceed to an empty processing phase.

**Processing failure:** If /ralph fails mid-batch, the queue preserves state. Individual insights resume from their failed phase on next /ralph invocation.

**Archive failure:** If archiving fails, the insights are still created and connected. Only the organizational cleanup is missing — re-run /archive-batch manually.
...
| After seed, before distill | /ralph 1 --batch {id} --type extract |
| After distill, during insights | /ralph --batch {id} (picks up from failed phase) |
| After all insights, before archive | /archive-batch {id} |
...
**Large source (2500+ lines):** /distill handles chunking automatically. The pipeline does not need special handling.
...
- Process insights inline instead of via /ralph subagents
- Archive a batch with incomplete tasks

**always:**
- Report progress at each phase boundary
- Validate all tasks are done before archiving
