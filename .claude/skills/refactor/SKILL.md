...
|--------|-------------------|-------------|
| **Granularity shift** | Insight templates, extraction depth in /distill, processing skills, context file "Insight Design" section | Template body length guidance, extraction granularity settings, composability test thresholds |
| **Organization shift** | Folder structure, domain map hierarchy, context file "Folder Architecture" section, hub domain map | Directory layout, domain map tier count, navigation references |
...
| **Processing shift** | /distill depth settings, /connect pass count, pipeline skills, context file "Processing Pipeline" section, config.yaml processing.depth | Extraction thoroughness, connection evaluation depth, chaining mode |
| **Navigation shift** | domain map tier structure, hub domain map, context file "domain map" section, insight Topics footers | Number of domain map tiers, hub content, navigation instructions |
| **Maintenance shift** | /health threshold values, condition-based trigger settings, context file maintenance instructions | Check frequency conditions, stale insight thresholds, refresh trigger conditions |
...
Artifact: [file path]
  ...
  Content impact: [does this affect existing insights? how many?]
```

### Content Impact Assessment

Some changes affect only infrastructure (skills, templates, context file). Others affect existing content (insights, domain maps):

| Change Type | Content Impact | Action Required |
|-------------|---------------|----------------|
| Template field addition | New insights get field, old insights don't | Add field to old insights (schema migration) |
| Template field removal | Old insights have unused field | Remove field from old insights (optional) |
| Folder restructure | Insights must move | Move files, update all wiki links |
| domain map tier change | domain map hierarchy restructured | Merge/split domain maps, update Topics footers |
| Enum value change | Old insights have invalid values | Update values in affected insights |

For content-impacting changes:
- Count affected insights: `grep -rl '[old value]' insights/*.md | wc -l`
...
2. **Soft warns:** What friction points does the new configuration create?
   ...
   - If soft warns exist: insight them in the plan with compensating mechanisms
...
Detected [N] dimension changes:
  ...
  [dimension]: [old] -> [new]
    ...
    Content impact: [N] existing insights need [what]
    Risk: [low/medium/high]
...
|-----------------|---------------------|
| Granularity | /distill, /validate, /audit |
| Processing | /distill, /connect, /refresh, /validate, /ralph, /pipeline |
| Linking | /connect, /refresh |
| Navigation | /connect (domain map update logic) |
...
5. Validate surrounding sections are unchanged
...
1. **Schema migration:** Add/remove/rename fields across insights
   ```bash
   # Example: add a new required field to all insights
...
3. **Link updates:** Update all wiki links affected by renames
   ...
   ops/scripts/rename-insight.sh "old name" "new name"
   ```

4. **domain map restructuring:** Merge/split domain maps as needed
   - For splits: create sub-domain maps, move Core Ideas, update parent
   - For merges: combine Core Ideas, remove redundant domain map, update insights' Topics footers
...
|-------|-----|----------------|
| Wiki link resolution | Scan for all `[[X]]` and validate X.md exists | Zero dangling links |
| Schema compliance | Check all insights against template _schema blocks | All required fields present, enum values valid |
| domain map hierarchy | Validate hub links to domain domain maps, domains link to topics | All tiers connected |
| Session orient | Simulate session start — can the context file be loaded? | No file-not-found errors |
| Three-space boundaries | Check insights vs ops vs self boundaries | No content in wrong space |
| Vocabulary consistency | Check that skills and context file use updated vocabulary | No stale universal terms |
| Skill integrity | Validate regenerated skills have valid frontmatter and structure | All skills parseable |
...
--=={ refactor complete }==--
  ...
  Content migration:
    [N] insights updated with new schema fields
    [N] wiki links updated
    [N] domain maps restructured
...
### Large Content Migration (100+ insights)

For large migrations:
1. Show the count and estimated time
2. Offer batch processing: "This affects 150 insights. Process all at once or in batches of 50?"
3. For batch processing, audit after each batch before continuing
...
| parallel_workers | Update /ralph to serial-only mode | Validate tmux available, update /ralph for parallel mode |
