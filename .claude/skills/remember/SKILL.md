---
name: remember
description: Capture friction as methodology insights. Three modes — explicit description, contextual (review recent corrections), session mining (scan transcripts for patterns). Triggers on "/remember", "/remember [description]".
version: "1.0"
...
3. **`ops/methodology/`** — read existing methodology insights before creating new ones (prevents duplicates)
...
User provides a description: `/remember "don't process personal insights like research"` or `/remember always check for duplicates before creating`
...
|----------|-------------|
| processing | How to extract, distill, or handle content |
| capture | How to record, file, or organize incoming material |
| connection | How to find, evaluate, or add links between insights |
...
| quality | Standards for insights, descriptions, titles |

### Step 2: Check for Existing Methodology Insights

Before creating a new insight, read all files in `ops/methodology/`:

```bash
ls -1 ops/methodology/*.md 2>/dev/null
```

For each existing insight, check if it covers the same behavioral area. Specifically:
- Does an existing insight address the same friction?
- Would the new learning extend an existing insight rather than warrant a new one?

| Check Result | Action |
|-------------|--------|
| No existing insights in this area | Create new methodology insight |
| Existing insight covers different aspect of same area | Create new insight, link to existing |
| Existing insight covers same friction | Extend existing insight with new evidence instead of creating duplicate |
| Existing insight contradicts new friction | Create both a new methodology insight AND an observation about the contradiction |

### Step 3: Create Methodology Insight

Write to `ops/methodology/`:

**Rule Zero:** This methodology insight becomes part of the system's canonical specification. ops/methodology/ is not a log of what happened — it is the authoritative declaration of how the system should behave. Write this insight as a directive: what the agent SHOULD do, not what went wrong. Future sessions, /rethink drift checks, and meta-skills will consult this insight as ground truth for system behavior.

**Filename:** Convert the prose title to kebab-case. Example: "don't process personal insights like research" becomes `dont-process-personal-insights-like-research.md`.

```markdown
---
description: [what this methodology insight teaches — specific enough to be actionable]
type: methodology
...
[The specific anti-pattern this insight prevents. What was the agent doing wrong?]
...
**Writing quality for methodology insights:**
- Be specific enough that a fresh agent session could follow this guidance without additional context
- Use concrete examples where possible — "when processing therapy insights" not "when processing certain types of content"
- State both the DO and the DON'T — methodology insights that only say what to do miss the anti-pattern that triggered them
- Keep scope explicit — unbounded methodology insights get applied where they should not be

### Step 4: Update Methodology domain map

Edit `ops/methodology.md` (create if missing):

1. Find the section for the insight's category
2. Add the insight with a context phrase: `- [[insight title]] — [what this teaches]`
3. If no section exists for this category, create one

```markdown
## [Category]

- [[existing insight]] — what it teaches
- [[new insight]] — what this teaches
```

### Step 5: Check Pattern Threshold

Count methodology insights in the same category:

```bash
grep -rl "^category: [CATEGORY]" ops/methodology/ 2>/dev/null | wc -l | tr -d ' '
```

If 3+ insights exist in the same category, this is a signal for /rethink:
...
--=={ remember }==--
  ...
  Updated: ops/methodology.md domain map
  Category: [category]
...
The agent reviews the current conversation to find corrections the user made that should become methodology insights.
...
|------------|-------------------|---------|
| Direct correction | "no", "that's wrong", "not like that", "incorrect" | "No, don't split that into separate insights" |
...
--=={ remember — contextual }==--
  ...
  Capture this as a methodology insight? (yes / no / modify)
```

**Wait for user confirmation.** Do not create insights from inferred corrections without approval — the agent might misinterpret what the user meant.

### Step 3: Handle Response

| Response | Action |
|----------|--------|
| "yes" | Create methodology insight (same process as explicit mode, `source: contextual`) |
...
| User provides additional context | Incorporate into the methodology insight |
...
```
  ...
  Capture all as methodology insights? (all / select numbers / none)
```
...
| Escalation patterns | User moving from gentle correction to firm direction | Methodology insight urgency signal |
...
|-------------|--------|------|
| Actionable methodology learning | Methodology insight in `ops/methodology/` | Clear behavioral change needed. Agent can act on this. |
| Novel observation requiring more context | Observation insight in `ops/observations/` | Pattern detected but not yet clear enough for methodology guidance. Needs accumulation. |

### Step 4: Deduplicate Against Existing Insights

Before creating any insights:

1. Read all existing methodology insights in `ops/methodology/`
2. Read all existing observations in `ops/observations/`
3. For each finding:
   - If an existing methodology insight covers this → skip or add as evidence to existing
   - If an existing observation covers this → skip or add as evidence to existing
   - If novel → create new insight
...
--=={ remember — mine }==--
  ...
  Methodology insights created: [count]
  ...
  Duplicates skipped: [count]
    - [existing insight] — already covers [pattern]

  Sessions marked as mined: [list]

  [If pattern thresholds reached:]
  Category "[category]" now has [N] methodology insights.
...
Work happens
  ...
  → /remember captures correction as methodology insight
  → methodology insight filed to ops/methodology/
  → agent reads methodology insights at session start (via context file reference)
  → agent behavior improves
  → fewer corrections needed
  → when methodology insights accumulate (3+ in same category)
...
The loop is healthy when methodology insights accumulate slowly (friction is being addressed) and /rethink elevates patterns to context-level changes when thresholds are exceeded.

The loop is unhealthy when the same category keeps getting methodology insights without elevation (the system is capturing friction but not learning from it).
...
- Every methodology insight you create becomes part of the spec. Write directives, not incident reports.
- The title should be an actionable behavior ("check for semantic duplicates before creating any insight") not a problem description ("duplicate creation issue").
- Future /rethink sessions will compare system behavior against what methodology insights declare. Vague insights create unmeasurable specs.

**What this means for the system:**
- ops/methodology/ is consulted by meta-skills (/ask, /architect, /rethink) as the source of truth for how the system works.
- Drift detection compares methodology insight assertions against actual config.yaml and context file state.
- When methodology insights are stale (older than config changes), the system surfaces this as a maintenance condition.

The methodology folder is the spec. /remember writes the spec. /rethink enforces the spec. The loop is closed.

---

## Methodology Insight Design

### Title Pattern

Methodology insight titles should describe what the agent should DO, not what went wrong:

| Bad (describes problem) | Good (describes behavior) |
|------------------------|--------------------------|
| "duplicate creation issue" | "check for semantic duplicates before creating any insight" |
| "wrong tone problem" | "match the user's formality level in all output" |
| "processing too aggressive" | "differentiate personal insights from research in processing depth" |

The title is what the agent reads at session start. It should be immediately actionable as a behavioral directive.

### Body Quality

Methodology insights are operational guidance, not essays. They should be:
...
| Category | Use When |
|----------|---------|
| processing | Friction during /distill, extraction, insight creation |
| capture | Friction during inbox filing, raw material handling |
| connection | Friction during /connect, link evaluation, domain map updates |
| maintenance | Friction during /refresh, health checks, cleanup |
...
| quality | Friction about insight quality, description writing, title crafting |
...
Create it and the `ops/methodology.md` domain map:

```markdown
---
description: Methodology insights capturing how this system has learned to operate
type: moc
---

# methodology

Methodology insights organized by category. Each insight captures a specific behavioral learning.
...
If a methodology insight with very similar content already exists:
1. Do NOT create a duplicate
2. Link to the existing insight
3. Add the new instance as evidence: update the existing insight's body with the new context
4. Report: "Extended existing methodology insight [[title]] with additional evidence"

### Contradicting Existing Methodology

If the new friction CONTRADICTS an existing methodology insight (user now wants the opposite of what was previously captured):
1. Create an observation in `ops/observations/` documenting the contradiction
2. Update the existing methodology insight's status to `superseded` and add `superseded_by: [new insight]`
3. Create the new methodology insight with the updated guidance
...
- User manually edits a insight the agent created (the edit reveals what was wrong)
- User chooses a different approach without explaining why
- User skips a step the agent suggested

In contextual mode, flag these as lower-confidence findings and always confirm before creating methodology insights from implicit signals.
