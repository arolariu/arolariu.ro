...
1. **`ops/derivation-manifest.md`** — vocabulary mapping
   ...
   - Use `vocabulary.cmd_connect` / `vocabulary.cmd_refresh` / `vocabulary.cmd_validate` for phase command names
...
| Pipeline queue | Automated processing state — what the SYSTEM needs to process | Pipeline skills | Automatic: /distill, /ralph, /connect |
...
If `ops/tasks.md` does not exist, insight: "No task stack found. Run `/tasks add [description]` to create one."
...
Discoveries are captured during pipeline work (e.g., /distill insights a connection opportunity, /connect notices a split candidate). They accumulate here until the user decides to convert them to tasks or discard them.
...
- `/distill` adds tasks to the queue AND insights discoveries in tasks.md
...
## Discoveries
- Interesting connection between [[insight A]] and [[insight B]] found during /distill
- domain map [[topic]] might need splitting (40+ insights observed during /connect)
```
