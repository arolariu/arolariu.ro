 ...
 */
export default async function fetchAppConfiguration(appEnvironment: ApplicationEnvironment): Promise<ApplicationConfiguration> {
  ...
  switch (appEnvironment) {
    ...
    case "DEVELOPMENT":
      {
        ...
        const settings = await client.getConfigurationSetting({ key: "CommonOptions:FrontendSettings", label: "DEVELOPMENT" });
    ...
    case "PRODUCTION":
      {
        const client = new AppConfigurationClient(endpoint, credentials, { retryOptions: { maxRetries: 5 } });
        const settings = await client.getConfigurationSetting({ key: "CommonOptions:FrontendSettings", label: "PRODUCTION" });
        const environment = JSON.parse(settings.value as string) as ApplicationConfiguration;
        const jwtSecret = await client.getConfigurationSetting({ key: "AuthOptions:Secret" });
        environment.API_JWT = jwtSecret.value as string;
        return environment;
