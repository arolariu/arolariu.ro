...
export async function fetchBFFUserFromAuthService(): Promise<Readonly<UserInformation>> {
  ...
  try {
    ...
    if (isAuthenticated) {
      ...
      if (!jwtSecret) throw new Error("JWT signing secret is not configured; cannot issue authentication token.");
      ...
      if (!guestJwtSecret) throw new Error("JWT signing secret is not configured; cannot issue guest token.");
