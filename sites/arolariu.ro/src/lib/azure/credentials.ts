 ...
 *
 * This function is server-only — enforced by the "server-only" import marker.
 */
export function getAzureCredential(): TokenCredential {
  if (!cachedCredential) {
    try {
      ...
      throw new Error("Azure credential initialization failed. Ensure AZURE_CLIENT_ID is set or Azure CLI is authenticated.", {cause});
    }
  }
