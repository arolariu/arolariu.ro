...
test.describe("Authentication @auth", () => {
  test.describe("Auth Page", () => {
    test(tagged("should load correctly", TEST_TYPE_TAGS.SMOKE, PRIORITY_TAGS.P1, FEATURE_TAGS.AUTH), async ({safeNavigate, page}) => {
    ...
    test(tagged("should display authentication component", TEST_TYPE_TAGS.E2E, FEATURE_TAGS.AUTH), async ({safeNavigate, page}) => {
    ...
    test(tagged("should pass accessibility checks", TEST_TYPE_TAGS.A11Y, FEATURE_TAGS.AUTH), async ({safeNavigate, checkA11y}) => {
      await safeNavigate("/auth/");
  ...
  test.describe("Protected Routes", () => {
    test(
      tagged("should redirect unauthenticated users from invoice domain", TEST_TYPE_TAGS.E2E, FEATURE_TAGS.AUTH),
    ...
    test(
      tagged("should handle dashboard access for unauthenticated users", TEST_TYPE_TAGS.E2E, FEATURE_TAGS.AUTH),
      async ({safeNavigate, page}) => {
...
test.describe("Invoice Domain (Authenticated) @invoices", () => {
  ...
  test.describe.skip("Authenticated Invoice Access", () => {
    test(
      tagged("should load invoice list page when authenticated", TEST_TYPE_TAGS.E2E, FEATURE_TAGS.AUTH),
      async ({safeNavigate, page}) => {
    ...
    test(tagged("should navigate to create invoice page", TEST_TYPE_TAGS.E2E, FEATURE_TAGS.AUTH), async ({safeNavigate, page}) => {
      await safeNavigate("/domains/invoices/");
