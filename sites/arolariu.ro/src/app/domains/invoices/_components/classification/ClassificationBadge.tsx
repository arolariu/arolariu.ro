...
import {ClassificationOrigin} from "@/types/invoices";
 ...
 *
 * @param props - {@link ClassificationBadgeProps}
 * @returns The classification badge element.
 */
export default function ClassificationBadge({
  classification,
}: Readonly<ClassificationBadgeProps>): React.JSX.Element {
  const t = useTranslations();

  const isManual = classification.origin === ClassificationOrigin.Manual;

  const hierarchyPath = classification.hierarchy
    .map((node) => node.officialLabel)
    .join(" → ");

  return (
    <div className={styles["badge"]} data-origin={classification.origin}>
      ...
      <div className={styles["header"]}>
        ...
        <span
          className={`${styles["origin"] ?? ""} ${isManual ? (styles["originManual"] ?? "") : (styles["originAnalysis"] ?? "")}`}
        >
          {isManual
      ...
      {classification.hierarchy.length > 0 && (
        <div className={styles["hierarchy"]}>
          <span className={styles["hierarchyTitle"]}>
            {t((m) => m.dialogs.invoices.classificationPicker.badge.hierarchy)}
          </span>
