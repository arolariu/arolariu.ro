 ...
 * 2. Stale-response protection: old searches cannot overwrite newer results or
 *    reopen results after the query is cleared or shortened.
...
const {searchClassifications} = await import("@/app/domains/invoices/_actions/analysis/searchClassifications");
...
describe("ClassificationPicker", () => {
  ...
  it("does not update results when a stale slow search resolves after a newer search", async () => {
    ...
    expect(screen.getByRole("option", {name: new RegExp(RESULT_B.code)})).toBeInTheDocument();
    ...
    expect(screen.queryByRole("option", {name: new RegExp(RESULT_A.code)})).not.toBeInTheDocument();
    expect(screen.getByRole("option", {name: new RegExp(RESULT_B.code)})).toBeInTheDocument();
  });

  it("does not reopen results when an in-flight search resolves after the query is cleared", async () => {
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});

    type SuccessResult = {success: true; data: readonly ClassificationSearchResult[]};
    let resolveSearch!: (value: SuccessResult) => void;
    mockSearch.mockImplementationOnce(
      () =>
        new Promise<SuccessResult>((resolve) => {
    ...
    await act(async () => {
      resolveSearch({success: true, data: [RESULT_A]});
      await Promise.resolve();
    });
  ...
  it("does not reopen results when an in-flight search resolves after the query is shortened", async () => {
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
  ...
  it("does not reopen results when an in-flight search resolves after the selection is cleared", async () => {
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
    ...
    expect(mockSearch).toHaveBeenCalledTimes(1);
    await user.click(screen.getByRole("button", {name: /classificationPicker\.clear/i}));
  ...
  it("calls onChange with exactly {system, code} when ArrowDown then Enter is pressed", async () => {
    ...
    mockSearch.mockImplementation(() => Promise.resolve({success: true as const, data: [RESULT_A, RESULT_B]}));
  ...
  it("sets aria-expanded to false and hides the listbox when Escape is pressed", async () => {
    ...
    mockSearch.mockImplementation(() => Promise.resolve({success: true as const, data: [RESULT_A]}));
  ...
  it("supports bounded ArrowDown and ArrowUp navigation and reopens available results", async () => {
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
    mockSearch.mockResolvedValue({success: true, data: [RESULT_A, RESULT_B]});
    ...
    expect(screen.getByRole("option", {name: new RegExp(RESULT_A.code)})).toHaveAttribute("aria-selected", "true");

    await user.keyboard("{ArrowDown}{ArrowDown}");
    expect(screen.getByRole("option", {name: new RegExp(RESULT_B.code)})).toHaveAttribute("aria-selected", "true");

    await user.keyboard("{ArrowUp}{ArrowUp}");
    expect(screen.getByRole("option", {name: new RegExp(RESULT_A.code)})).toHaveAttribute("aria-selected", "true");
  });

  it("keeps the listbox closed when search fails or returns no matches", async () => {
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
    mockSearch
      .mockResolvedValueOnce({success: false, error: {code: "SERVER_ERROR", message: "Search failed"}})
  ...
  it("selects an option with a pointer interaction", async () => {
    const onChange = vi.fn<(selection: ClassificationSelection | null) => void>();
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
    mockSearch.mockResolvedValue({success: true, data: [RESULT_A]});
    ...
    await user.click(screen.getByRole("option", {name: new RegExp(RESULT_A.code)}));
  ...
  it.each(["{Enter}", " "])("supports %s activation when an option has direct focus", async (activationKey) => {
    const onChange = vi.fn<(selection: ClassificationSelection | null) => void>();
    const user = userEvent.setup({advanceTimers: vi.advanceTimersByTime});
    mockSearch.mockResolvedValue({success: true, data: [RESULT_A]});
    ...
    const option = screen.getByRole("option", {name: new RegExp(RESULT_A.code)});
    option.focus();
  ...
  it("disables both the combobox and clear control", () => {
    render(
      <ClassificationPicker
        system={SYSTEM}
        value={{system: SYSTEM, code: RESULT_A.code}}
    ...
    expect(screen.getByRole("button", {name: /classificationPicker\.clear/i})).toBeDisabled();
  });
  ...
  it(`renders at most ${MAX_RESULTS} options regardless of how many the action returns`, async () => {
    ...
    const oversizedResults = Array.from({length: 75}, (_, i) => makeResult(`code-${String(i).padStart(3, "0")}`, `Label ${i}`));

    mockSearch.mockImplementation(() => Promise.resolve({success: true as const, data: oversizedResults}));
