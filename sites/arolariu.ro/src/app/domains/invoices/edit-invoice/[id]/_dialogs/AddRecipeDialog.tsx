...
export default function AddRecipeDialog(): React.JSX.Element {
  ...
  const handleCreate = useCallback(async () => {
    try {
      ...
      toast.success(t((m) => m["IMS--Dialogs"].recipeDialog.create.success));
      close();
      router.refresh();
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      toast.error(message || t((m) => m["IMS--Dialogs"].recipeDialog.create.error));
    }
        ...
        <form className={styles["formBody"]}>
          <div className={styles["fieldGroup"]}>
            <div className={styles["fieldHeader"]}>
              <Label htmlFor='recipe-add-name'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.recipeName)}</Label>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger
                    render={
          ...
          <div className={styles["fieldGroup"]}>
            <Label htmlFor='recipe-add-description'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.description)}</Label>
            <Textarea
          ...
          <div className={styles["fieldGroup"]}>
            <Label htmlFor='recipe-add-difficulty'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.difficulty)}</Label>
            <Select
          ...
          <div className={styles["fieldGroup"]}>
            <div className={styles["fieldHeader"]}>
              <Label htmlFor='recipe-add-instructions'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.instructions)}</Label>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger
                    render={
          ...
          <div className={styles["timeGrid"]}>
            <div className={styles["fieldGroup"]}>
              <Label htmlFor='recipe-add-preparation-time'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.prepTime)}</Label>
            ...
            <div className={styles["fieldGroup"]}>
              <Label htmlFor='recipe-add-cooking-time'>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.cookTime)}</Label>
              <div className={styles["timeRow"]}>
          ...
          {(Number(recipe.preparationTime) > 0 || Number(recipe.cookingTime) > 0) && (
            <div className={styles["fieldGroup"]}>
              <Label>{t((m) => m["IMS--Dialogs"].recipeDialog.fields.totalDuration)}</Label>
              <div className={styles["timeRow"]}>
                <TbToolsKitchen3 className={styles["mutedIcon"]} />
                <span>
                  {Number(recipe.preparationTime) + Number(recipe.cookingTime)} {t((m) => m["IMS--Dialogs"].recipeDialog.minutes)}
                </span>
              </div>
            </div>
          )}
        ...
        <DialogFooter className={styles["dialogFooter"]}>
          <div className={styles["footerActions"]}>
            ...
            <Button
              ...
              {isAdding ? t((m) => m["IMS--Dialogs"].recipeDialog.buttons.saving) : t((m) => m["IMS--Dialogs"].recipeDialog.buttons.save)}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
