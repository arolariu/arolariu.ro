...
export default function CreateInvoiceDialog({
	...
	onOpenChange,
	selectedScans,
}: Readonly<CreateInvoiceDialogProps>): React.JSX.Element {
	...
	const totalSize = selectedScans.reduce((acc, scan) => acc + scan.sizeInBytes, 0);
	...
	const handleCreate = async (): Promise<void> => {
		...
		try {
			...
			// Mark scans as used by invoice(s) and archive them
			if (result.convertedScanIds.length > 0 && result.invoices.length > 0) {
				// For batch mode, all scans point to the single invoice
				// For single mode, each scan points to its own invoice
				if (mode === "batch") {
					...
					// The order of invoices matches the order of scans
				...
				<AnimatePresence mode='wait'>
					...
					{step === "complete" && (
						<motion.div
							...
							<h3 className='text-lg font-semibold text-gray-900 dark:text-white'>
								{createdCount > 1
									? t("complete.titlePlural", {count: String(createdCount)})
									: t("complete.title", {count: String(createdCount)})}
							</h3>
							<p className='mt-1 text-sm text-gray-500 dark:text-gray-400'>
								{createdCount > 1 ? t("complete.descriptionPlural") : t("complete.description")}
							</p>
							...
							<div className='mt-6 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20'>
								<p className='text-sm text-green-700 dark:text-green-300'>
									...
									{createdCount > 1
										? t("complete.nextStepsDescriptionPlural")
										: t("complete.nextStepsDescription")}
								</p>
							</div>
							...
							<DialogFooter className='mt-6 justify-center'>
								<Button
									...
									{createdCount > 1 ? t("complete.viewButtonPlural") : t("complete.viewButton")}
									<TbArrowRight className='ml-2 h-4 w-4' />
								</Button>
							</DialogFooter>
						</motion.div>
