...
/** Two scans uploaded — minimal multi-scan state. */
export const TwoScans: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: "Post-upload prompt with two scans. Tests minimal multi-scan thumbnail rendering and prompt layout.",
      },
    },
  },
};

/** Very many scans (15) — overflow thumbnail test. */
export const VeryManyScans: Story = {
  args: {
    isVisible: true,
    completedScans: Array.from({length: 15}, (_, i) => ({
      id: `scan-${i + 1}`,
      preview: storyImageScanUrl,
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Post-upload prompt with 15 scans. Tests thumbnail overflow rendering, max-display limit (first 5 + counter), and layout with many scans.",
      },
    },
  },
};

/** Empty scans array — edge case guard. */
export const NoScans: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Post-upload prompt with empty completedScans array. Tests edge case rendering when no scans are available (should handle gracefully).",
      },
    },
  },
};
