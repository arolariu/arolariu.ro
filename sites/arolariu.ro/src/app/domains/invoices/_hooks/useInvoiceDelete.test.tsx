...
describe("useInvoiceDelete", () => {
  ...
  it("calls deleteInvoice, toasts success, removes the entity, and runs onComplete", async () => {
    ...
    await act(async () => {
      await result.current.performDelete(invoice);
    });
  ...
  it("performDeleteBulk loops, aggregates counters, and toasts the summary", async () => {
    ...
    await act(async () => {
      bulk = await result.current.performDeleteBulk(["a", "b", "c"]);
    });
    ...
    await act(async () => {
      bulk = await result.current.performDeleteBulk(["success-only"]);
    });
    ...
    await act(async () => {
      bulk = await result.current.performDeleteBulk(["failure-only"]);
    });
