...
const originalRecipe: Recipe = {
  ...
  preparationTime: 10,
  cookingTime: 20,
  referenceForMoreDetails: "https://example.com/original",
};
...
const updatedRecipe: Recipe = {
  ...
  preparationTime: 15,
  cookingTime: 30,
  referenceForMoreDetails: "https://example.com/updated",
};

const mockInvoice = {
  id: "inv-1",
  possibleRecipes: [keepRecipe, originalRecipe],
} as Invoice;
...
describe("useRecipeUpdate", () => {
  ...
  it("replaces the named recipe and patches the invoice", async () => {
    const updated = {...mockInvoice, possibleRecipes: [keepRecipe, updatedRecipe]};
    vi.mocked(patchInvoice).mockResolvedValueOnce({success: true, invoice: updated});

    const {result} = renderHook(() => useRecipeUpdate());

    let returned: Invoice | undefined;
    await act(async () => {
      returned = await result.current.updateRecipeCallback("Original", updatedRecipe);
    });
    ...
    expect(patchInvoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          possibleRecipes: [keepRecipe, updatedRecipe],
        }),
      }),
    );
  ...
  it("throws when patchInvoice reports failure", async () => {
    ...
    await act(async () => {
      try {
        await result.current.updateRecipeCallback("Original", updatedRecipe);
      } catch (error) {
        caught = error;
      }
    });
