<script lang="ts">
	...
	const palettes: Record<Color, { solid: string; soft: string; outline: string }> = {
		...
		purple: {
			...
			outline:
				'text-purple-600 dark:text-purple-400 border border-purple-500/40 hover:bg-purple-500/10'
		},
		pink: {
		...
		orange: {
			...
			outline:
				'text-orange-600 dark:text-orange-400 border border-orange-500/40 hover:bg-orange-500/10'
		},
		gray: {
			...
			outline:
				'text-gray-700 dark:text-gray-300 border border-gray-400/40 dark:border-gray-600 hover:bg-gray-500/10'
		}
	};
...
<span class={classes} {title} aria-label={title || text} {style}>{text}</span>
