...
<Header sticky showNavLinks={false} {actionsConfig} />
...
<main id="main-content" class="min-h-screen transition-colors duration-300">
	<div class="max-w-6xl mx-auto p-6">
		...
		>
			<div
				class="px-4 py-2 border-b transition-colors duration-300 bg-gray-100 dark:bg-gray-800 border-gray-200 dark:border-gray-700"
			>
				<div class="flex items-center justify-between">
					<span
						class="text-sm font-mono transition-colors duration-300 text-gray-600 dark:text-gray-400"
						>alexandru-olariu-cv.json</span
					>
			...
			<div class="p-4 overflow-auto max-h-[70vh]">
				<pre
					class="text-sm font-mono whitespace-pre-wrap break-words transition-colors duration-300 text-gray-900 dark:text-gray-100">{activeTab ===
					'formatted'
						? formattedJSON
						: rawJSON}</pre>
			</div>
		...
		<div
			class="mt-6 rounded-lg p-4 border transition-all duration-300 bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-700"
		>
			<h3 class="text-lg font-semibold mb-2 text-blue-700 dark:text-blue-300">
				JSON Resume Schema
			</h3>
			<p class="text-sm mb-3 transition-colors duration-300 text-gray-600 dark:text-gray-400">
				This JSON follows the standardized JSON Resume schema v1.0.0, making it compatible with
				various resume builders, parsers, and ATS systems that support this format.
			</p>
