...
{
  /// <summary>
  /// Initializes a legacy compatibility shape used by older endpoint tests that passed a domain scan directly.
  /// </summary>
  /// <param name="UserIdentifier">Unused client-supplied owner identifier retained only for compatibility.</param>
  /// <param name="InitialScan">Optional single initial scan to wrap into the current scans collection.</param>
  /// <param name="Metadata">Optional client metadata.</param>
  public CreateInvoiceRequestDto(
    Guid UserIdentifier,
    InvoiceScan? InitialScan,
    IDictionary<string, object>? Metadata)
    : this(
      Name: "Invoice",
      Description: null,
      ...
      MerchantReference: null,
      IsImportant: false,
      Scans: InitialScan is { } scan ? [new CreateInvoiceScanRequestDto(scan.Type, scan.Location, scan.Metadata)] : [],
      Items: null,
      Metadata: Metadata)
  {
    _ = UserIdentifier;
