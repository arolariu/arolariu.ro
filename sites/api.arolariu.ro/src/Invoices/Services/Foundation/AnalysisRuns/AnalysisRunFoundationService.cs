...
{
  private readonly IDatabaseBroker analysisRunBroker;
  ...
  public AnalysisRunFoundationService(
    IDatabaseBroker analysisRunBroker,
  ...
  await TryCatchAsync(async () =>
  {
    ...
    await analysisRunBroker
      .EnsureAnalysisQueueAsync(cancellationToken)
  ...
  await TryCatchAsync(async () =>
  {
    ...
    var created = await analysisRunBroker
      .CreateAnalysisRunAsync(run, cancellationToken)
      .ConfigureAwait(false);
    return created;
  }).ConfigureAwait(false);
  ...
  await TryCatchNullableAsync(async () =>
  {
    ...
    await foreach (var candidate in analysisRunBroker
      .StreamAnalysisRunClaimCandidatesAsync(now, cancellationToken)
      .ConfigureAwait(false))
    {
      ...
      try
      {
        var persisted = await analysisRunBroker
          .ReplaceAnalysisRunAsync(claimed, candidate.ETag, cancellationToken)
          .ConfigureAwait(false);
  ...
  await TryCatchAsync(async () =>
  {
    ...
    var counts = await analysisRunBroker
      .CountPendingAnalysisRunsByTargetTypeAsync(now, cancellationToken)
      .ConfigureAwait(false);
  ...
  await TryCatchAsync(async () =>
  {
    ...
    var existingRun = await analysisRunBroker.ReadAnalysisRunAsync(runId, cancellationToken).ConfigureAwait(false);
    ...
    var replacedRun = await analysisRunBroker
      .ReplaceAnalysisRunAsync(renewedRun, run.ETag, cancellationToken)
      .ConfigureAwait(false);
    return replacedRun;
  }).ConfigureAwait(false);
  ...
  await TryCatchAsync(async () =>
  {
    ...
    var replacedRun = await analysisRunBroker
      .ReplaceAnalysisRunAsync(completedRun, run.ETag, cancellationToken)
      .ConfigureAwait(false);
    return replacedRun;
  }).ConfigureAwait(false);
  ...
  await TryCatchAsync(async () =>
  {
    ...
    var replacedRun = await analysisRunBroker
      .ReplaceAnalysisRunAsync(failedRun, run.ETag, cancellationToken)
      .ConfigureAwait(false);
    return replacedRun;
  }).ConfigureAwait(false);
}
