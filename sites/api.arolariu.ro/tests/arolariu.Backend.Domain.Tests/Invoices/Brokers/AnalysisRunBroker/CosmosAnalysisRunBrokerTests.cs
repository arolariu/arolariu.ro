...
public sealed class CosmosAnalysisRunBrokerTests
{
  ...
  public async Task EnsureContainerAsync_ContainerAlreadyHasNegativeOneTtl_DoesNotCallReplaceContainer()
  {
    ...
    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.EnsureContainerAsync));
  ...
  /// <summary>Verifies that <see cref="CosmosAnalysisRunBroker.CreateAsync"/> rejects a null run with <see cref="ArgumentNullException"/> before touching Cosmos.</summary>
  [TestMethod]
  public async Task CreateAsync_NullRun_ThrowsArgumentNullException()
  {
    await Assert.ThrowsExactlyAsync<ArgumentNullException>(
      () => broker.CreateAsync(null!, CancellationToken.None).AsTask()).ConfigureAwait(true);
  }

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.CreateAsync"/> returns the created run stamped with the
  /// response's ETag on success, and records the Cosmos RU charge under the <c>"create"</c> operation tag.
  /// </summary>
  [TestMethod]
  public async Task CreateAsync_Success_ReturnsRunWithETagAndRecordsCharge()
  {
    ...
    var result = await broker.CreateAsync(run, CancellationToken.None).ConfigureAwait(true);
  ...
  /// <see cref="CosmosAnalysisRunBroker.CreateAsync"/> call enriches the underlying activity with Cosmos DB context
  /// and marks it successful, covering the "non-null activity" side of every <c>activity?.</c> branch in this method.
  /// </summary>
  [TestMethod]
  public async Task CreateAsync_Success_RecordsActivityWhenListenerPresent()
  {
    var run = AnalysisRunTestBuilder.Queued();
    var mockResponse = new Mock<ItemResponse<AnalysisRun>>();
    mockResponse.Setup(r => r.Resource).Returns(run);
    ...
    await broker.CreateAsync(run, CancellationToken.None).ConfigureAwait(true);

    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.CreateAsync));
    Assert.IsNotNull(activity);
    Assert.AreEqual(ActivityStatusCode.Ok, activity.Status);
  }
  ...
  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.ReadAsync"/> returns the resolved run stamped with the
  /// response's ETag on success, and records the Cosmos RU charge under the <c>"read"</c> operation tag.
  /// </summary>
  [TestMethod]
  public async Task ReadAsync_Found_ReturnsRunWithETagAndRecordsCharge()
  {
    var run = AnalysisRunTestBuilder.Queued();
    var mockResponse = new Mock<ItemResponse<AnalysisRun>>();
    mockResponse.Setup(r => r.Resource).Returns(run);
    ...
    var result = await broker.ReadAsync(run.Id, CancellationToken.None).ConfigureAwait(true);
  ...
  /// <see cref="CosmosAnalysisRunBroker.ReadAsync"/> call enriches the underlying activity with Cosmos DB context
  /// and marks it successful, covering the "non-null activity" side of the found-path <c>activity?.</c> branches.
  /// </summary>
  [TestMethod]
  public async Task ReadAsync_Found_RecordsActivityWhenListenerPresent()
  {
    var run = AnalysisRunTestBuilder.Queued();
    var mockResponse = new Mock<ItemResponse<AnalysisRun>>();
    mockResponse.Setup(r => r.Resource).Returns(run);
    ...
    await broker.ReadAsync(run.Id, CancellationToken.None).ConfigureAwait(true);

    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.ReadAsync));
    Assert.IsNotNull(activity);
    Assert.AreEqual(ActivityStatusCode.Ok, activity.Status);
  }

  /// <summary>
  /// Verifies that a Cosmos 404 (NotFound) surfaced by <see cref="CosmosAnalysisRunBroker.ReadAsync"/> is treated as
  ...
  /// </summary>
  [TestMethod]
  public async Task ReadAsync_WhenCosmos404_ReturnsNull()
  {
    ...
    var result = await broker.ReadAsync(runId, CancellationToken.None).ConfigureAwait(true);
  ...
  /// surfaced by <see cref="CosmosAnalysisRunBroker.ReadAsync"/> still marks the underlying activity successful,
  /// covering the "non-null activity" side of the NotFound-specific catch block's own success recording.
  /// </summary>
  [TestMethod]
  public async Task ReadAsync_WhenCosmos404_RecordsActivityWhenListenerPresent()
  {
    var runId = Guid.NewGuid();
    mockContainer
      .Setup(c => c.ReadItemAsync<AnalysisRun>(runId.ToString(), It.IsAny<PartitionKey>(), It.IsAny<ItemRequestOptions>(), It.IsAny<CancellationToken>()))
    ...
    await broker.ReadAsync(runId, CancellationToken.None).ConfigureAwait(true);
  ...
  /// <see cref="CosmosAnalysisRunBroker.ReadAsync"/> is translated into <see cref="AnalysisRunLeaseConflictException"/>
  /// via this method's own (second) catch block, distinct from the NotFound-specific short-circuit above.
  /// </summary>
  [TestMethod]
  public async Task ReadAsync_WhenCosmos412_ThrowsAnalysisRunLeaseConflictException()
  {
    var runId = Guid.NewGuid();
    mockContainer
      .Setup(c => c.ReadItemAsync<AnalysisRun>(runId.ToString(), It.IsAny<PartitionKey>(), It.IsAny<ItemRequestOptions>(), It.IsAny<CancellationToken>()))
    ...
    var exception = await Assert.ThrowsExactlyAsync<AnalysisRunLeaseConflictException>(
      () => broker.ReadAsync(runId, CancellationToken.None).AsTask()).ConfigureAwait(true);
  ...
  #region ReplaceAsync Tests

  /// <summary>Verifies that <see cref="CosmosAnalysisRunBroker.ReplaceAsync"/> rejects a null run with <see cref="ArgumentNullException"/> before touching Cosmos.</summary>
  [TestMethod]
  public async Task ReplaceAsync_NullRun_ThrowsArgumentNullException()
  {
    await Assert.ThrowsExactlyAsync<ArgumentNullException>(
      () => broker.ReplaceAsync(null!, "\"etag-1\"", CancellationToken.None).AsTask()).ConfigureAwait(true);
  }

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.ReplaceAsync"/> rejects a null expected ETag with
  ...
  /// </summary>
  [TestMethod]
  public async Task ReplaceAsync_NullExpectedETag_ThrowsArgumentNullException()
  {
    var run = AnalysisRunTestBuilder.Queued();

    await Assert.ThrowsExactlyAsync<ArgumentNullException>(
      () => broker.ReplaceAsync(run, null!, CancellationToken.None).AsTask()).ConfigureAwait(true);
  }

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.ReplaceAsync"/> rejects an empty or whitespace-only expected
  ...
  [DataRow("")]
  [DataRow("   ")]
  public async Task ReplaceAsync_BlankExpectedETag_ThrowsArgumentException(string expectedETag)
  {
    var run = AnalysisRunTestBuilder.Queued();

    await Assert.ThrowsExactlyAsync<ArgumentException>(
      () => broker.ReplaceAsync(run, expectedETag, CancellationToken.None).AsTask()).ConfigureAwait(true);
  }

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.ReplaceAsync"/> returns the replaced run stamped with the
  /// response's ETag on success, and records the Cosmos RU charge under the <c>"replace"</c> operation tag.
  /// </summary>
  [TestMethod]
  public async Task ReplaceAsync_Success_ReturnsRunWithETagAndRecordsCharge()
  {
    var run = AnalysisRunTestBuilder.Queued();
    var mockResponse = new Mock<ItemResponse<AnalysisRun>>();
    mockResponse.Setup(r => r.Resource).Returns(run);
    ...
    var result = await broker.ReplaceAsync(run, "\"etag-1\"", CancellationToken.None).ConfigureAwait(true);
  ...
  /// <summary>
  /// Verifies that, when an <see cref="System.Diagnostics.ActivityListener"/> is present, a successful
  /// <see cref="CosmosAnalysisRunBroker.ReplaceAsync"/> call enriches the underlying activity with Cosmos DB context
  /// and marks it successful, covering the "non-null activity" side of every <c>activity?.</c> branch in this method.
  /// </summary>
  [TestMethod]
  public async Task ReplaceAsync_Success_RecordsActivityWhenListenerPresent()
  {
    var run = AnalysisRunTestBuilder.Queued();
    var mockResponse = new Mock<ItemResponse<AnalysisRun>>();
    mockResponse.Setup(r => r.Resource).Returns(run);
    ...
    await broker.ReplaceAsync(run, "\"etag-1\"", CancellationToken.None).ConfigureAwait(true);

    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.ReplaceAsync));
    Assert.IsNotNull(activity);
    Assert.AreEqual(ActivityStatusCode.Ok, activity.Status);
  }
  ...
  #region StreamClaimCandidatesAsync Tests

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.StreamClaimCandidatesAsync"/> yields candidates from every
  ...
  /// </summary>
  [TestMethod]
  public async Task StreamClaimCandidatesAsync_MultiplePages_YieldsCandidatesInOrderAndRecordsChargePerPage()
  {
    ...
    await foreach (var candidate in broker.StreamClaimCandidatesAsync(DateTimeOffset.UtcNow, CancellationToken.None).ConfigureAwait(true))
    {
    ...
    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.StreamClaimCandidatesAsync));
    Assert.IsNotNull(activity);
    Assert.AreEqual(ActivityStatusCode.Ok, activity.Status);
  }
  ...
  [TestMethod]
  public async Task StreamClaimCandidatesAsync_EmptyPage_YieldsNoCandidatesButRecordsCharge()
  {
  ...
  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.StreamClaimCandidatesAsync"/> observes cancellation before
  /// reading the first page, propagating <see cref="OperationCanceledException"/> unchanged rather than swallowing it.
  /// </summary>
  [TestMethod]
  public async Task StreamClaimCandidatesAsync_CancellationRequested_ThrowsOperationCanceledException()
  {
    ...
    {
      await foreach (var _ in broker.StreamClaimCandidatesAsync(DateTimeOffset.UtcNow, cts.Token).ConfigureAwait(true))
      {
      }
    }).ConfigureAwait(true);
  }
  ...
  /// </summary>
  [TestMethod]
  public async Task StreamClaimCandidatesAsync_CosmosNotFoundOnReadNext_ThrowsAnalysisRunNotFoundExceptionWithGenericMessage()
  {
    ...
    {
      await foreach (var _ in broker.StreamClaimCandidatesAsync(DateTimeOffset.UtcNow, CancellationToken.None).ConfigureAwait(true))
      {
      }
    }).ConfigureAwait(true);
  ...
  #region CountPendingByTargetTypeAsync Tests

  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.CountPendingByTargetTypeAsync"/> aggregates the grouped
  ...
  /// </summary>
  [TestMethod]
  public async Task CountPendingByTargetTypeAsync_MultipleTargetTypesAcrossPages_ReturnsAggregatedCountsAndRecordsCharge()
  {
    ...
    var counts = await broker.CountPendingByTargetTypeAsync(DateTimeOffset.UtcNow, CancellationToken.None).ConfigureAwait(true);
    ...
    var activity = activityRecorder.FindActivity(nameof(CosmosAnalysisRunBroker.CountPendingByTargetTypeAsync));
    Assert.IsNotNull(activity);
    Assert.AreEqual(ActivityStatusCode.Ok, activity.Status);
  }
  ...
  [TestMethod]
  public async Task CountPendingByTargetTypeAsync_NoResults_ReturnsEmptyDictionary()
  {
  ...
  /// <summary>
  /// Verifies that <see cref="CosmosAnalysisRunBroker.CountPendingByTargetTypeAsync"/> observes cancellation before
  /// reading the first page, propagating <see cref="OperationCanceledException"/> unchanged rather than swallowing it.
  /// </summary>
  [TestMethod]
  public async Task CountPendingByTargetTypeAsync_CancellationRequested_ThrowsOperationCanceledException()
  {
    ...
    await Assert.ThrowsExactlyAsync<OperationCanceledException>(
      () => broker.CountPendingByTargetTypeAsync(DateTimeOffset.UtcNow, cts.Token).AsTask()).ConfigureAwait(true);
  }
  ...
  /// </summary>
  [TestMethod]
  public async Task CountPendingByTargetTypeAsync_CosmosPreconditionFailedOnReadNext_ThrowsAnalysisRunLeaseConflictExceptionWithGenericMessage()
  {
    SetUpPendingCountFailure(MakeCosmosException(HttpStatusCode.PreconditionFailed));

    var exception = await Assert.ThrowsExactlyAsync<AnalysisRunLeaseConflictException>(
      () => broker.CountPendingByTargetTypeAsync(DateTimeOffset.UtcNow, CancellationToken.None).AsTask()).ConfigureAwait(true);
  ...
  /// </summary>
  [TestMethod]
  public async Task ReplaceAsync_WhenCosmos429ProvidesRetryAfter_PreservesRetryAfterFromException()
  {
    var run = AnalysisRunTestBuilder.Queued();
  ...
  /// </summary>
  [TestMethod]
  public async Task CreateAsync_WhenCosmosStatusCodeIsUnhandled_RethrowsOriginalCosmosException()
  {
    var run = AnalysisRunTestBuilder.Queued();
  ...
  /// <see cref="CosmosAnalysisRunBroker"/>. It is resolved via reflection because it is <see langword="private"/> to
  ...
  private static readonly Type PendingCountProjectionType =
    typeof(CosmosAnalysisRunBroker).GetNestedType("PendingCountProjection", BindingFlags.NonPublic)
  ...
  /// <see cref="CosmosAnalysisRunBroker"/> are given innocuous defaults.
  /// </summary>
