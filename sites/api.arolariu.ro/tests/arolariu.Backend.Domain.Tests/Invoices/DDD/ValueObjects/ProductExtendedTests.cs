...
public sealed class ProductExtendedTests
{
  ...
  /// <summary>
  /// Verifies merge precedence and preservation of server-owned workflow metadata.
  /// </summary>
  [TestMethod]
  public void Merge_RightValuesProvided_ReturnsMergedProductWithLeftMetadata()
  {
    StandardClassification leftClassification = ClassificationTestData.Gpc("10000123", "Left");
    StandardClassification rightClassification = ClassificationTestData.Gpc("10000456", "Right");
    AllergenAssessment leftAssessment = AllergenAssessment.NoSignals(Guid.NewGuid());
    AllergenAssessment rightAssessment = AllergenAssessment.Insufficient(Guid.NewGuid());
    var left = new Product
    {
      Name = "Left",
      Classification = leftClassification,
      Quantity = 1,
      QuantityUnit = "left-unit",
      ProductCode = "left-code",
      Price = 1,
      AllergenAssessment = leftAssessment,
    ...
    var right = new Product
    {
      Name = "Right",
      Classification = rightClassification,
      Quantity = 2,
      QuantityUnit = "right-unit",
      ProductCode = "right-code",
      Price = 3,
      AllergenAssessment = rightAssessment,
      Metadata = new ProductMetadata
      {
        IsEdited = false,
        IsComplete = false,
    ...
    Product merged = Product.Merge(left, right);

    Assert.AreNotSame(left, merged);
    Assert.AreNotSame(right, merged);
    Assert.AreEqual("Right", merged.Name);
    Assert.AreSame(rightClassification, merged.Classification);
    Assert.AreEqual(2, merged.Quantity);
    Assert.AreEqual("right-unit", merged.QuantityUnit);
    Assert.AreEqual("right-code", merged.ProductCode);
    Assert.AreEqual(3, merged.Price);
    Assert.AreSame(rightAssessment, merged.AllergenAssessment);
    Assert.IsTrue(merged.Metadata.IsEdited);
    Assert.IsTrue(merged.Metadata.IsComplete);
    Assert.IsTrue(merged.Metadata.IsSoftDeleted);
    Assert.AreEqual(0.95, merged.Metadata.Confidence);
    Assert.AreEqual("Left", left.Name);
    Assert.AreSame(leftClassification, left.Classification);
    Assert.AreEqual(1, left.Quantity);
    Assert.AreEqual("left-unit", left.QuantityUnit);
    Assert.AreEqual("left-code", left.ProductCode);
    Assert.AreEqual(1, left.Price);
    Assert.AreSame(leftAssessment, left.AllergenAssessment);
    Assert.IsFalse(left.Metadata.IsEdited);
    Assert.AreEqual("Right", right.Name);
    Assert.AreSame(rightClassification, right.Classification);
    Assert.AreSame(rightAssessment, right.AllergenAssessment);
    Assert.IsFalse(right.Metadata.IsEdited);
    Assert.IsFalse(right.Metadata.IsComplete);
    Assert.IsFalse(right.Metadata.IsSoftDeleted);
    Assert.AreEqual(0.1, right.Metadata.Confidence);
  }

  /// <summary>
  /// Verifies null analysis values retain left values without mutating either merge input.
  /// </summary>
  [TestMethod]
  public void Merge_RightAnalysisValuesNull_RetainsLeftAnalysisAndDoesNotMutateInputs()
  {
    StandardClassification leftClassification = ClassificationTestData.Gpc("10000123", "Left");
    AllergenAssessment leftAssessment = AllergenAssessment.NoSignals(Guid.NewGuid());
    var left = new Product
    {
      ...
      Classification = leftClassification,
      AllergenAssessment = leftAssessment,
      Metadata = new ProductMetadata { IsEdited = false, IsComplete = true },
    };
    var right = new Product
    {
      Name = "Right",
      ...
      Metadata = new ProductMetadata { IsEdited = false, IsComplete = false },
    };

    Product merged = Product.Merge(left, right);

    Assert.AreSame(leftClassification, merged.Classification);
    Assert.AreSame(leftAssessment, merged.AllergenAssessment);
    Assert.IsFalse(left.Metadata.IsEdited);
    Assert.IsTrue(left.Metadata.IsComplete);
    Assert.IsFalse(right.Metadata.IsEdited);
    Assert.IsFalse(right.Metadata.IsComplete);
    Assert.IsNull(right.Classification);
    Assert.IsNull(right.AllergenAssessment);
  }
