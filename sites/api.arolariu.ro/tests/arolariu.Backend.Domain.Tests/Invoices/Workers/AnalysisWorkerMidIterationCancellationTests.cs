...
public sealed class AnalysisWorkerMidIterationCancellationTests
{
  ...
  public async Task ExecuteAsync_ProcessingObservesCancellationMidIteration_ReturnsWithoutFaulting()
  {
    ...
    services.AddScoped<IAnalysisProcessingService>(_ => processing);
  ...
  /// </summary>
  private sealed class CancellingDuringIterationProcessingService(CancellationTokenSource cancellationSource) : IAnalysisProcessingService
  {
    ...
    /// <inheritdoc/>
    public Task EnsureAnalysisStoreAsync(CancellationToken cancellationToken) => Task.CompletedTask;
